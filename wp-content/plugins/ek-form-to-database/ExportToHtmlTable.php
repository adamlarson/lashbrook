...
class ExportToHtmlTable extends ExportBase implements CFDBExport {
    ...
    public function export($formName, $options = null) {
        ...
        <?php
        $pluginUrl = plugins_url('/', __FILE__);
        wp_enqueue_style('ekoverride.css', $pluginUrl . 'css/ekoverride.css');
        ?>
        <div id="ekoverride-container">
                ...
                <thead>
                    ...
                    if (isset($this->options['header']) && $this->options['header'] != 'true') {
                        ...
                        <tr>
                            <?php if ($canDelete) { ?>
                                <th id="delete_th">
                                    <button id="delete" name="delete"><?php echo htmlspecialchars(__('Delete', 'contact-form-7-to-database-extension')) ?></button>
                        ...
                        <tr>
                            ...
                            foreach ($this->dataIterator->getDisplayColumns() as $aCol) {
                                $cell = $this->rawValueToPresentationValue(
                                        $this->dataIterator->row[$aCol], $showLineBreaks, ($fields_with_file && in_array($aCol, $fields_with_file)), $this->dataIterator->row[$submitTimeKeyName], $formName, $aCol);
        ...
        <?php // EK adjustment to add confirmation to delete specific rows ?>
        <script>
            jQuery(function() {
                jQuery("table").on("click", "#delete", function(e) {
                    var res = confirm("Are you sure you want to delete the checked entries?");
