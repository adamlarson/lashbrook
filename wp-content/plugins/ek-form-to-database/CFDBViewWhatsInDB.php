...
class CFDBViewWhatsInDB extends CFDBView {
        ...
        if ($currSelection) {
            ...
            if (isset($_POST['delete']) &&
                ...
                if (isset($_POST['submit_time'])) {
                    ...
                    $wpdb->query(
                            $wpdb->prepare(
                                    "delete from `$tableName` where `form_name` = '%s' and `submit_time` = %F", $currSelection, $submitTime));
                    ...
                    foreach ($_POST as $name => $value) { // checkboxes
                        if ($value == 'row') {
                            ...
                            $wpdb->query(
                                    $wpdb->prepare(
                                            "delete from `$tableName` where `form_name` = '%s' and `submit_time` = %F", $currSelection, $name));
                        }
                    }
                }
                    ...
                    <?php if ($currSelection) { ?>
                        ...
                        <form name="exportcsv" action="">
                            ...
                            <!--<span style="font-size: x-small;"><br /><?php
                                echo '<a href="admin.php?page=' . $plugin->getShortCodeBuilderPageSlug() . '">' .
                    ...
                    <?php if ($currSelection && $canEdit) { ?>
                        <form action="" method="post">
                            ...
                            <input name="delete" type="submit"
                                   value="<?php echo htmlspecialchars(__('Delete all records in this form', 'contact-form-7-to-database-extension')); ?>"
    ...
    protected function paginationDiv($plugin, $totalRows, $rowsPerPage, $page) {
        ...
        printf(__('Returned entries %s to %s of %s entries in the database', 'contact-form-7-to-database-extension'), $startRow, $endRow, $totalRows);
        echo '</span>';
