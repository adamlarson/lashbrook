...
//        $savedVersion = $this->getVersionSaved();
    ...
    public function delete_wpcf7_fields($formName) {
        ...
        $wpdb->query($wpdb->prepare(
                        ...
                        "` where `form_name` = '%s' and `field_name` in ('_wpcf7', '_wpcf7_version', '_wpcf7_unit_tag', '_wpnonce', '_wpcf7_is_ajax_call', '_wpcf7_captcha_challenge_captcha')", $formName));
    }
...
//            }
            foreach ($cf7->posted_data as $name => $value) {
                ...
                if ($cf7->uploaded_files && isset($cf7->uploaded_files[$nameClean])) {
                    ...
                    if ($filePath) {
                        ...
                        $didSaveFile = $wpdb->query($wpdb->prepare($parametrizedFileQuery, $time, $title, $nameClean, $valueClean, $order++, $content));
                        if (!$didSaveFile) {
                ...
                if (!$didSaveFile) {
                    $wpdb->query($wpdb->prepare($parametrizedQuery, $time, $title, $nameClean, $valueClean, $order++));
                }
            }
            ...
            if ($cf7->uploaded_files && is_array($cf7->uploaded_files)) {
                foreach ($cf7->uploaded_files as $field => $filePath) {
                    if (!in_array($field, $foundUploadFiles) &&
                        ...
                        $content = file_get_contents($filePath);
                        $didSaveFile = $wpdb->query($wpdb->prepare($parametrizedFileQuery, $time, $title, $field, $fileName, $order++, $content));
                        if (!$didSaveFile) {
            ...
            if ($this->getOption('SaveCookieData', 'false') == 'true' && is_array($_COOKIE)) {
                ...
                foreach ($_COOKIE as $cookieName => $cookieValue) {
                    if (empty($saveCookies) || $this->fieldMatches($cookieName, $saveCookies)) {
                        $wpdb->query($wpdb->prepare($parametrizedQuery, $time, $title, 'Cookie ' . $cookieName, $cookieValue, $order++));
                    }
                }
            }
            ...
            if ($user) {
                ...
                $wpdb->query($wpdb->prepare($parametrizedQuery, $time, $title, 'Submitted Login', $user, $order));
            }

            // Capture the IP Address of the submitter
            $order = ($order < 10000) ? 10000 : $order + 1; // large order num to try to make it always last
            $wpdb->query($wpdb->prepare($parametrizedQuery, $time, $title, 'Submitted From', $ip, $order));
        } catch (Exception $ex) {
            $this->getErrorLog()->logException($ex);
        }
    ...
    public function createAdminMenu() {
        ...
        add_menu_page($displayName, __('EK Form Entries', 'contact-form-7-to-database-extension'), $this->roleToCapability($roleAllowed), $menuSlug, //$this->getDBPageSlug(),
                array(&$this, 'whatsInTheDBPage'), 'dashicons-clipboard', 30.321);
...
//        }
        ...
        add_submenu_page($menuSlug, $displayName . ' Options', __('Options', 'contact-form-7-to-database-extension'), 'manage_options', get_class($this) . 'Settings', array(&$this, 'settingsPage'));
...
//                         array(&$this, 'showShortCodeBuilderPage'));
    ...
    public function whatsInTheDBPage() {
        if (isset($_REQUEST['submit_time'])) {
            ...
            <?php wp_nonce_field(); ?>
            <?php // EK - Addition of back link  ?>
                <div class="ek-details-back" style="margin-top: 20px;">
                    <a href="?page=CF7DBPluginSubmissions">Back</a>
                </div>
                <button id="delete" name="delete" style="margin-top: 20px;"><?php echo htmlspecialchars(__('Delete', 'contact-form-7-to-database-extension')); ?></button>
            </form>
            <?php // EK adjustment to add confirmation to delete this entry  ?>
            <script>
                jQuery(function() {
    ...
    public function getFileUrl($submitTime, $formName, $fieldName) {
        return sprintf('%s?action=cfdb-file&s=%s&form=%s&field=%s', admin_url('admin-ajax.php'), $submitTime, urlencode($formName), urlencode($fieldName));
    }
    ...
    public function addAdminNotices() {
        ...
        if (isset($editorData['Version'])) {
            if (version_compare($editorData['Version'], $requiredEditorVersion) == -1) {
                ...
                <?php echo htmlentities(__('Current version: ', 'contact-form-7-to-database-extension'));
                echo $editorData['Version']; ?><br/>
                <?php echo htmlentities(__('Minimum required version: ', 'contact-form-7-to-database-extension'));
                echo $requiredEditorVersion; ?><br/>
